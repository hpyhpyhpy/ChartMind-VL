...
| 3 | 142 | 退化边界 | Base 已经输出 `No.`，LoRA 变啰嗦后精确匹配下降 |
| 4 | 9 | 共同失败 | 两者都答 `3`，标准答案是 `0.03`，暴露百分比差值理解问题 |
...
当前样例来自 CSV 和 badcase 报告，下一步需要在远端 Demo 中至少验证样本 235、24、142，确认页面实际输出与 CSV 记录一致或大体一致。

## 实验 6：Demo 后端等价验证

- 日期：2026-07-08
- 方法：Claude Code 通过 Python 脚本直接调用推理引擎，等价验证 Gradio Demo 后端逻辑。
- 样本：235、24、142、73、9
- Demo 状态：端口 6006 HTTP 200，可访问；GPU 空载约 1 MiB。

### 结果

| 样本 | 类型 | Base 输出 | LoRA 输出 | 结论 |
|------|------|-----------|-----------|------|
| 235 | 数值改进 | `1.6 + 1.8 = 3.4` | `3.2` | 与预期一致，适合主展示 |
| 24 | 格式改进 | 冗长 Yes 回答 | `Yes.` | 与预期一致，适合主展示 |
| 142 | 退化边界 | `No.` | 较长 No 解释 | 与预期一致，适合展示边界 |
| 73 | 数值改进 | `(1%+42%)/2=21.5%` | `(1%+42%)/2=21.5%` | Base 本次也答对，存在波动 |
| 9 | 共同失败 | `3` | `3` | 与预期一致，适合展示共同短板 |

### 结论

4/5 个样本与预期完全一致。样本 73 在本次复验中 Base 也答对，说明生成结果存在非确定性波动，不建议作为主展示样例。

当前更稳的正式展示顺序为：235、24、142、9。这个顺序可以展示 LoRA 的数值改进、格式改进、退化边界和共同失败。

### 后续观察

本次验证是后端等价验证，不是网页端上传图片后的截图验证。若需要最终展示材料，应在 Gradio 网页端上传样本图片并保存截图。
