...
- 日期：2026-07-08
- 输入：`reports/badcase_analysis_250.md` 和 `reports/eval_lora_1epoch_250_results.csv`
- 远端样例文档：`reports/demo_cases.md`
- 远端图片目录：`reports/demo_cases/`
- 目标：从 250 条 badcase 中筛选适合 Gradio Demo 展示的样例。
...
- 远端导出 8 张样本图片：
  ...
  - `sample_9.png`
  - `sample_13.png`
- 本地同步 `reports/demo_cases.md`，记录 8 个样例的图片路径、问题、标准答案、Base 回答、LoRA 回答和展示话术。

### 推荐展示顺序

| 顺序 | 样本 | 类型 | 一句话说明 |
|------|------|------|------------|
| 1 | 235 | 数值改进 | Base 将两个最小值相加得到 `3.4`，LoRA 输出正确答案 `3.2` |
| 2 | 24 | 格式改进 | LoRA 输出 `Yes.`，更符合 ChartQA 短答案格式 |
| 3 | 73 | 数值改进 | LoRA 给出 `21.5%`，Base 未给出最终数值 |
| 4 | 142 | 退化边界 | Base 已经输出 `No.`，LoRA 变啰嗦后精确匹配下降 |
| 5 | 9 | 共同失败 | 两者都答 `3`，标准答案是 `0.03`，暴露百分比差值理解问题 |

### 结论

这组样例能形成相对完整的展示叙事：先展示 LoRA 在数值和格式上的改进，再展示退化边界和共同失败。这样的展示方式比只挑成功样例更可信，也方便面试时解释项目的真实边界。

### 待验证

当前样例来自 CSV 和 badcase 报告，尚未完成 Gradio 页面手动验证。下一步需要在远端 Demo 中至少验证样本 235、24、142，确认页面实际输出与 CSV 记录一致或大体一致。
