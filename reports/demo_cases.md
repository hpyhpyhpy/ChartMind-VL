...
| LoRA 回答 | (1% + 42%) / 2 = 21.5% ✅ |
| 展示用途 | **数值改进** |
| 一句话话术 | LoRA 在数值推理任务上学会了正确的计算步骤并输出精确结果，Base 模型则回答中断。 |
...
3. **样本 73** — 更复杂的数值推理改进
4. **样本 142** — 诚实地展示 LoRA 也有退化（Base 更好）
5. **样本 9** — 诚实地展示共同局限性（0.03 vs 3）

这个顺序从 "LoRA 有进步" -> "也有局限" -> "我们清楚问题在哪"，呈现一个平衡、可信的展示叙事。
