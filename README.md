...
基于 250 条 badcase 分析，远端已整理 `reports/demo_cases.md`，并导出 8 张样本图片到 `reports/demo_cases/`。Claude Code 已通过 Python 脚本直接调用 Demo 后端逻辑进行复验，4/5 个样本与预期一致，样本 73 出现生成波动。

| 顺序 | 样本 | 类型 | 展示话术 |
|------|------|------|----------|
| 1 | 235 | 数值改进 | Base 加法算错，LoRA 给出正确答案 `3.2` |
| 2 | 24 | 格式改进 | LoRA 输出 `Yes.`，比 Base 冗长回答更贴近 ChartQA 短答案 |
| 3 | 142 | 退化边界 | Base 输出 `No.`，LoRA 变长后从精确匹配退化为部分匹配 |
| 4 | 9 | 共同失败 | Base 和 LoRA 都答 `3`，标准答案为 `0.03`，体现百分比差值短板 |

样本 73 可作为候选补充：CSV 中 LoRA 输出 `21.5%`、Base 未给出最终数值，但本次后端复验中 Base 和 LoRA 都答对，说明它不适合作为稳定主展示样例。

这组样例用于形成平衡叙事：LoRA 有小幅改进，也有退化和共同失败边界。目前已完成后端等价验证，尚未完成网页端上传图片截图验证。
...
- 在 Gradio 网页端上传图片并保存截图，用于最终展示材料。
